...
class SoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def update_q_func(self, batch):
        ...
        with chainer.no_backprop_mode(), chainer.using_config('train', False):
            ...
            assert next_q.shape == entropy_term.shape, 'next_q.shape: {}, entropy_term.shape {}'.format(next_q.shape, entropy_term.shape)
    ...
    def act_and_train(self, obs, reward):
        ...
        if self.last_state is not None:
            ...
            self.logger.debug('len(replay_buffer) == {}'.format(len(self.replay_buffer)))
    ...
    def batch_observe_and_train(
        ...
        for i in range(len(batch_obs)):
            ...
            self.replay_updater.update_if_necessary(self.t)
        self.logger.debug('len(replay_buffer) == {}'.format(len(self.replay_buffer)))
        self.logger.debug('====== global timestep: {} ======'.format(self.t))
