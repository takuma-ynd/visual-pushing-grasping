...
from chainer.links.model.vision.vgg import VGGLayers
...
class VGGLayers(link.Chain):
    ...
    def __init__(self, pretrained_model=None, n_layers=16):
    ...
    def forward(self, x1, x2, layers=None, **kwargs):
        ...
        for key, funcs in self.functions.items():
            ...
            if key == 'fc_2nd_input':
                for func in funcs:
                    h2 = func(h2)
                continue
            if key == 'fc_2nd_input_merge':
                ## Dirtyyyy
                h = funcs[0](chainer.functions.concat((h, h2), axis=1))
                h = funcs[1](h)
                h = funcs[2](h)
                continue
...
class MyVGG16Layers(VGGLayers):
    ...
    def __init__(self, pretrained_model='auto'):
        super(MyVGG16Layers, self).__init__(pretrained_model, 16)
    ...
    def functions(self):
        return collections.OrderedDict([
            ...
            ('fc_2nd_input', [self.fc_2nd_input, relu, dropout]),
            ('fc_2nd_input_merge', [self.fc_2nd_input_merge, relu, dropout]),
