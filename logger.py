...
class Logger():
    ...
    def save_images(self, iteration, color_image, depth_image, mode, reset_counter=0):
        color_image = cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(self.color_images_directory, '%06d.%s.%03d.color.png' % (iteration, mode, reset_counter)), color_image)
    ...
    def save_heightmaps(self, iteration, color_heightmap, depth_heightmap, mode, reset_counter=0):
        ...
        cv2.imwrite(os.path.join(self.color_heightmaps_directory, '%06d.%s.%03d.color.png' % (iteration, mode, reset_counter)), color_heightmap)
